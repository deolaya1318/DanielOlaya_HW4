...
void solucion_u(float *matriz, float *vectorb, float *b_0, float *b_1, float *b_2);
void imprimir_resultados(float b_0, float b_1, float b_2);
...
int main(){
  ...
  solucion_u(U, b_prima, &b_0, &b_1, &b_2);
  imprimir_resultados(b_0, b_1, b_2);
  return 0;
}
...
// Un modelo que puede explicar un sistema fisico, se representa por una matriz de coeficientes que multiplica los parametros del modelo. En este caso se usa la primera ecuacion del enunciado para el punto 4, para conocer los coeficientes que multiplican los parametros b_0, b_1, b_2, que son funcion de theta.
...
void llenar_coeficientes(float *matriz_datos, float *coeficientes, float *vectorb, float *theta, int n_row){
  ...
  for (i = 0; i < n_row; i++){
    ...
    coeficientes[i * 3 + 1] = theta[i];
    coeficientes[i * 3 + 2] = pow(theta[i],2);
...
void imprimir_resultados(float b_0, float b_1, float b_2){
  ...
  char *archivo_final = "parametros_hipotesis2.dat";
